<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-50">
    <!-- Header Section -->
    <div class="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-100 sticky top-0 z-30">
          ...
          <div class="max-w-2xl mx-auto relative">
            <div class="flex shadow-lg rounded-2xl overflow-hidden transition-all duration-300 hover:shadow-xl">
              <div class="relative flex-1">
                <input
                  ...
                  class="w-full px-6 py-4 pr-12 border-0 focus:outline-none focus:ring-2 focus:ring-champagne-gold/50 text-lg transition-all"
                ...
                <div v-if="showSuggestions && filteredSuggestions.length > 0" 
                     class="absolute top-full left-0 right-0 bg-white border border-gray-100 rounded-2xl shadow-2xl z-50 mt-2 animate-slideDown">
                  <div class="py-2 max-h-64 overflow-y-auto">
                    <button 
                      v-for="suggestion in filteredSuggestions" 
                      :key="suggestion"
                      @mousedown="selectSuggestion(suggestion)"
                      class="w-full px-5 py-3 text-left hover:bg-champagne-gold/5 transition-all duration-200 text-sm flex items-center group"
                    >
                      <svg class="w-4 h-4 text-gray-400 mr-3 group-hover:text-champagne-gold transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                      </svg>
                      <span class="group-hover:text-champagne-gold transition-colors">{{ suggestion }}</span>
                    </button>
                  </div>
                </div>
              </div>
              ...
              <button 
                @click="performSearch"
                :disabled="loading"
                class="px-8 py-4 bg-gradient-to-r from-champagne-gold to-rich-bronze text-white hover:from-rich-bronze hover:to-champagne-gold focus:outline-none focus:ring-2 focus:ring-champagne-gold/50 disabled:opacity-50 transition-all duration-300 font-medium transform hover:scale-105 active:scale-95"
              >
            ...
            <div class="mt-6 flex flex-wrap justify-center gap-2">
              <button 
                ...
                class="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-full text-sm hover:bg-champagne-gold hover:text-white hover:border-champagne-gold transition-all duration-300 shadow-sm hover:shadow-md transform hover:-translate-y-0.5"
              >
                {{ suggestion }}
              </button>
            </div>
    ...
    <!-- Filters Section -->
    <div v-if="currentQuery" class="bg-white/60 backdrop-blur-md border-b border-gray-100">
          ...
          <div class="flex items-center space-x-4">
            ...
            <div class="relative">
              <button 
                ...
                class="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-200 rounded-xl hover:border-champagne-gold hover:shadow-md transition-all duration-300"
              >
              ...
              <div v-show="dropdownOpen === 'category'" class="absolute top-full left-0 mt-2 w-52 bg-white border border-gray-100 rounded-2xl shadow-2xl z-40 animate-slideDown overflow-hidden">
                <div class="py-2">
                  <button 
                    v-for="option in categoryOptions" 
                    :key="option.value"
                    @click="selectOption('category', option.value)"
                    class="w-full px-4 py-3 text-left text-sm hover:bg-champagne-gold/10 transition-all duration-200 flex items-center group"
                  >
            ...
            <div class="relative">
              ...
              <div v-show="dropdownOpen === 'style'" class="absolute top-full left-0 mt-2 w-52 bg-white border border-gray-100 rounded-2xl shadow-2xl z-40 animate-slideDown overflow-hidden">
                <div class="py-2">
                  <button 
            ...
            <button 
              ...
              @click="clearFilters"
              class="text-sm text-gray-500 hover:text-champagne-gold transition-all duration-300 font-medium"
            >
              Clear filters
            </button>
          </div>
    ...
    <div v-if="quotaExceeded" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
      <div class="bg-gradient-to-r from-champagne-gold/10 to-champagne-gold/5 border border-champagne-gold/20 rounded-2xl p-6 text-center backdrop-blur-sm">
        <div class="inline-flex items-center justify-center w-12 h-12 bg-champagne-gold/20 rounded-full mb-3">
          <svg class="w-6 h-6 text-champagne-gold" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
          </svg>
        </div>
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Taking a quick break</h3>
        <p class="text-gray-600">We've reached our search capacity for now. Please try again in a few moments!</p>
      </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="loading" class="flex justify-center items-center py-20">
        <div class="text-center">
          <div class="inline-block animate-spin rounded-full h-12 w-12 border-4 border-champagne-gold/20 border-t-champagne-gold mb-4"></div>
          <p class="text-gray-600 text-lg animate-pulse">Searching for styles...</p>
        </div>
      </div>
            ...
            <div class="p-4">
              ...
              <div class="flex space-x-2">
                <button 
                  ...
                  class="flex-1 px-3 py-2 text-xs font-medium text-gray-700 bg-gray-50 rounded-lg hover:bg-gray-100 transition-all duration-200"
                >
                  View
                </button>
                <button 
                  @click.stop="sendToWhatsApp(image)"
                  class="flex-1 px-3 py-2 text-xs font-medium text-white bg-gradient-to-r from-green-600 to-green-500 rounded-lg hover:from-green-700 hover:to-green-600 transition-all duration-200 transform hover:scale-105"
                >
                  Order
                </button>
              </div>
        ...
        <div v-if="totalPages > 1" class="mt-12 flex justify-center">
          <nav class="flex items-center space-x-2">
            <button 
              ...
              class="px-4 py-2 text-sm font-medium text-gray-600 bg-white border border-gray-200 rounded-xl hover:bg-champagne-gold hover:text-white hover:border-champagne-gold disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 shadow-sm hover:shadow-md"
            >
              Previous
            </button>
            ...
            <template v-for="page in getVisiblePages()" :key="page">
              ...
              <button 
                ...
                @click="goToPage(page)"
                :class="[
                  'px-4 py-2 text-sm font-medium rounded-xl transition-all duration-300 shadow-sm',
                  page === currentPage 
                    ? 'bg-gradient-to-r from-champagne-gold to-rich-bronze text-white font-semibold shadow-md transform scale-110' 
                    : 'text-gray-700 bg-white border border-gray-200 hover:bg-champagne-gold hover:text-white hover:border-champagne-gold hover:shadow-md'
                ]"
              >
                {{ page }}
              </button>
    ...
    <div v-if="showImageModal && selectedImage" 
      ...
      <div class="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl animate-scaleIn" @click.stop>
        <!-- Modal Header -->
        ...
        <div class="flex flex-col md:flex-row max-h-[calc(90vh-80px)]">
          ...
          <div class="md:w-80 p-4 space-y-4">
            ...
            <button 
              ...
              class="w-full bg-gradient-to-r from-green-600 to-green-500 text-white px-6 py-4 rounded-xl font-medium hover:from-green-700 hover:to-green-600 transition-all duration-300 flex items-center justify-center space-x-2 shadow-lg hover:shadow-xl transform hover:scale-105 active:scale-95"
            >
